...
@section('content')
    ...
    <div class="flex items-center gap-4 mb-6">
        ...
        <div>
            <h2 class="text-xl font-bold text-[#1A1A1A]">Edit Artikel</h2>
            <p class="text-xs text-[#9E9E9E] mt-0.5">Perbarui artikel panduan tani atau ubah status penerbitannya.</p>
        </div>
    </div>
    ...
    @if($errors->any())
        <div class="mb-5 flex items-start gap-3 bg-danger-50 border border-danger-400/30 text-danger-700 px-4 py-3 rounded-xl text-sm">
            ...
            <div>
                <p class="font-semibold text-xs">Pembaruan artikel gagal</p>
    ...
    <div class="bg-white rounded-2xl border border-[#E0E0E0] card-shadow overflow-hidden">
        <form method="POST" action="{{ route('admin.artikel.update', $article['id']) }}" enctype="multipart/form-data" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-5">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            >
                                @php
                                    $currentCat = old('category', $article['category'] ?? 'Tips Tani');
                                    $categories = ['Tips Tani', 'Hama & Penyakit', 'Cuaca & Musim', 'Teknologi Tani', 'Pupuk & Tanah', 'Lainnya'];
                                @endphp
                        ...
                        <div>
                            ...
                            >
                                @php
                                    $currentStatus = old('status', $article['status'] ?? 'Publikasi');
                                @endphp
                                <option value="Publikasi" {{ $currentStatus == 'Publikasi' ? 'selected' : '' }}>Publikasi</option>
                                <option value="Draf" {{ $currentStatus == 'Draf' ? 'selected' : '' }}>Draf</option>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            @php
                                $currentDate = old('published_at');
                                if (empty($currentDate) && !empty($article['published_at'])) {
                                    $currentDate = date('Y-m-d\TH:i', strtotime($article['published_at']));
                                }
                            @endphp
                            <input
                                type="datetime-local"
                ...
                <div class="space-y-4">
                    ...
                    <div x-data="imagePreview('{{ $article['image_url'] ?? '' }}')" class="space-y-3">
                        {{-- Image Display --}}
                        <div class="w-full aspect-[16/9] rounded-2xl bg-[#F8F9FA] border-2 border-dashed border-[#E0E0E0] overflow-hidden flex items-center justify-center relative group">
                            ...
                            <div x-show="!imageUrl" class="flex flex-col items-center justify-center p-6 text-center">
                                ...
                                <span class="text-xs text-[#9E9E9E] font-medium">Klik atau tarik file ke sini untuk mengunggah thumbnail baru</span>
                            </div>
                        ...
                        <p class="text-[11px] text-[#9E9E9E] leading-relaxed">
                            Pilih gambar baru jika ingin mengganti gambar saat ini. Ukuran maksimal 2 MB.
                        </p>
                    </div>
                </div>
    ...
    {{-- Alpine image script --}}
    <script>
        function imagePreview(defaultUrl) {
            return {
                imageUrl: defaultUrl || '',
                fileChosen(event) {
